...
> = Interfaces extends `${infer Name}${"," | "&"}${infer Rest}`
  ? MergeInterface<
      ...
      Known,
      Types &
        (Trim<Name> extends infer Key extends keyof Known ? Known[Key] : never)
    >
  : Types &
...
export type RemoveComment<T extends string> = RemoveDocumentComment<
  RemoveMultiLineComment<RemoveSingleLineComment<T>>
>;
...
type RemoveDocumentComment<T extends string> =
  T extends `${infer First}"${infer Comment}"\n${infer Rest}`
...
>(
  ...
  const query = JSON.stringify(
    ...
    (key, value) => {
      ...
      if (typeof value === "object" && "select" in value && !("where" in value))
        return value.select;
